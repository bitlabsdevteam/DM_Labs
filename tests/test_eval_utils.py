...
class ScheduleComparisonRemapTests(unittest.TestCase):
    ...
    def test_schedule_comparison_reports_shared_source_timestep_keys(self):
        ...
        self.assertIn("quality_summary", comparison)
        self.assertEqual(comparison["quality_summary"]["primary_metric"], "timestep_auc_pseudo_perplexity")
        self.assertTrue(comparison["quality_summary"]["normalized_timestep_remapping"])
        self.assertIn("notes", comparison["quality_summary"])
        self.assertIn("warnings", comparison["quality_summary"])
        self.assertIn("normalized timestep fraction", " ".join(comparison["quality_summary"]["notes"]))
        self.assertIn("not yet decisive", " ".join(comparison["quality_summary"]["warnings"]))
